...
class InvoiceWorkflowV9():
    ...
    def paid_invoice(self):
        def pay_single_invoice(data_update, wizard_context):
            data = self.payement_obj.default_get(
                [
                    "communication",
                    "currency_id",
                    "invoice_ids",
        ...
        invoice_to_paid = self.invoice_obj.search_read([(self.field, 'in', self.status_map['paid']), (
            'state', '=', 'open'), ('type', '=', 'out_invoice')], [self.paid_date, 'date_invoice'])
        ...
        for i, invoice in enumerate(invoice_to_paid):
            ...
            rpc_thread.spawn_thread(
                pay_single_invoice, [
                    data_update, wizard_context], {})
        rpc_thread.wait()

    def rename(self, name_field):
        invoice_to_paid = self.invoice_obj.search_read(
            [
                (name_field,
                 '!=',
                 False),
        ...
        for i, invoice in enumerate(invoice_to_paid):
            ...
            rpc_thread.spawn_thread(
                self.invoice_obj.write, [
                    invoice['id'], {
                        'number': invoice[name_field], name_field: False}], {})
        rpc_thread.wait()
